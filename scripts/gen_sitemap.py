...
def get_total_count():
    ...
    res = requests.request(
        "GET", url=BASE_URL + END_POINT, headers=HEADERS, params=queries
    ).json()
...
def get_sitemap_materials():
    ...
    for i in range(cnt):
        offset = i * 50
        queries = {"fields": "id,updatedAt", "limit": limit, "offset": offset}
        res = requests.request(
            "GET", url=BASE_URL + END_POINT, headers=HEADERS, params=queries
        )
        for obj in res.json()["contents"]:
            jst_date_string = get_jst_date_string(obj["updatedAt"])
            elm = {"loc": f'{HOST_URL}/post/{obj["id"]}', "lastmod": jst_date_string}
...
def job():
    ...
    tree.write(
        Path(BASE_DIR, "public", "sitemap.xml"), encoding="utf-8", xml_declaration=True
    )
    success_message = """
    ============
