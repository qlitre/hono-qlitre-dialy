...
export const Pagination = ({
  ...
  const getPaginationItem = (p: number) => {
    if (p === currentPage) {
      return <span class={activeClass}>{p}</span>;
    }
    return (
      <a class={linkClass} href={getPath(p)}>
        {p}
      </a>
    );
  };
  ...
  return (
    <div class={wrapperClass}>
      <ul class={pagerClass}>
        ...
        {currentPage >= 4 && <li class={pageClass}>{getPaginationItem(1)}</li>}
        ...
        {currentPage <= numPages - 3 && (
          <li class={pageClass}>{getPaginationItem(numPages)}</li>
        )}
